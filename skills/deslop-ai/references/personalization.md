...
Accept only explicitly supplied files or folders. Never crawl drives. Store profiles and run artifacts under the private DeSlopAI root, outside the plugin and Git repository. Do not store raw source text, source paths, filenames, email addresses, per-document hashes, per-document metrics, or unapproved excerpts in a profile.

## 2. Profile construction

Maintain separate consulting and academic profiles. Extract anonymous aggregate features: sentence/paragraph distributions, punctuation, headings, bullets, transitions, hedging, vocabulary, citations, text density, and deterministic editorial-risk dimensions.
